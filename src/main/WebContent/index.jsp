...
function loadMyTasks() {
    fetch('taskApi?type=mine&projectID=' + KANBAN_PROJECT_ID)
        .then(r => r.json())
        .then(tasks => {
...
function loadRecentChat() {
    ...
    fetch('ChatServlet?action=getRooms&projectID=' + DASH_PROJECT_ID)
        ...
        .then(rooms => {
            ...
            if (!teamRoom) return;
            // 2. 메시지 5개 조회
            return fetch('ChatServlet?action=getMessages&projectID=' + DASH_PROJECT_ID + '&roomId=' + teamRoom.roomId + '&limit=5&type=recent');
        })
        .then(r => r && r.json())
        .then(messages => {
            if (!messages) return;
