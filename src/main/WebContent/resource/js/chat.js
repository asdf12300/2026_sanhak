...
function loadMessages(roomId, firstLoad = false) {
    ...
    fetch(
        `ChatServlet?action=getMessages&projectID=${projectId}&roomId=${roomId}&limit=${messageLimit}&offset=${messageOffset}`
    )
...
function loadRoomInfo(roomId) {
    fetch(`ChatServlet?action=getRoomInfo&projectID=${projectId}&roomId=${roomId}`)
        .then(response => response.text())
        .then(text => {
...
function uploadAndSendImage(file) {
    ...
    fetch(`ChatServlet?projectID=${projectId}`, { method: 'POST', body: formData })
        .then(r => r.json())
        .then(data => {
