...
public class ChatServlet extends HttpServlet {
    ...
    private S3FileStorage storage = new S3FileStorage();
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("professor".equals(user.getRole())) {
            ...
            response.sendRedirect("index.jsp" + (projectId != null ? "?projectID=" + projectId : ""));
            return;
        }
        if ("getRooms".equals(action)) {
            ...
            viewImage(request, response, user);
        } else {
  ...
  // 팀 채팅방 생성
    ...
    private void uploadImage(HttpServletRequest request, HttpServletResponse response, LoginDTO user)
        ...
        try {
            ...
            int roomId = Integer.parseInt(request.getParameter("roomId"));
            if (!chatDAO.isRoomMember(roomId, user.getId())) {
                response.getWriter().write("{\"success\": false, \"message\": \"채팅방 권한이 없습니다\"}");
                return;
            }
            ChatRoomDTO room = chatDAO.getChatRoom(roomId);
            if (room == null || room.getProjectId() != projectId) {
                response.getWriter().write("{\"success\": false, \"message\": \"채팅방 정보가 올바르지 않습니다\"}");
                return;
            }

            String savedName = UUID.randomUUID().toString() + ext;
            String s3Key = "projects/" + projectId + "/chat/" + roomId + "/" + savedName;
            try (InputStream input = filePart.getInputStream()) {
                storage.upload(input, s3Key, filePart.getContentType());
            }

            String imageUrl = request.getContextPath()
                    + "/ChatServlet?action=image&projectID=" + projectId
                    + "&roomId=" + roomId
                    + "&key=" + java.net.URLEncoder.encode(s3Key, "UTF-8");
    ...
    private void viewImage(HttpServletRequest request, HttpServletResponse response, LoginDTO user)
            throws IOException {
        try {
            int projectId = Integer.parseInt(request.getParameter("projectID"));
            int roomId = Integer.parseInt(request.getParameter("roomId"));
            ...
            ChatRoomDTO room = chatDAO.getChatRoom(roomId);
            String expectedPrefix = "projects/" + projectId + "/chat/" + roomId + "/";
            if (room == null || room.getProjectId() != projectId || !key.startsWith(expectedPrefix)) {
                response.sendError(HttpServletResponse.SC_FORBIDDEN);
                return;
            }
            ...
            storage.download(key, response.getOutputStream());
        } catch (Exception e) {
            e.printStackTrace();
