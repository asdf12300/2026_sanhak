...
public class ListDAO {
    ...
    public List<ProjectDTO> getMyProjectsWithFolder(String userId) {
        List<ProjectDTO> list = new ArrayList<>();
        String sql = "SELECT DISTINCT b.id, b.title, b.content, b.deadline, b.team_leader, b.created_at, b.folder_id " +
                     "FROM board b " +
                     "LEFT JOIN project_member pm ON b.id = pm.project_id " +
                     "WHERE b.team_leader = ? OR (pm.member_id = ? AND pm.status = 'accepted') " +
                     "ORDER BY b.id DESC";
        try (Connection conn = DBConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                dto.setTeam_leader(rs.getString("team_leader"));
                dto.setCreated_at(rs.getTimestamp("created_at"));
                dto.setFolderId(rs.getInt("folder_id"));
                list.add(dto);
            }
